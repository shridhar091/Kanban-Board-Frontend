...
// ✅ Add task without reload
const addTaskToSection = async (taskData) => {
  try {
    const newTask = await createTask(taskData); // get saved task from API
    props.section.taskIds.push(newTask); // directly update section's array
    showTaskModal.value = false;
  } catch (err) {
    console.error("Failed to add task", err);
  }
